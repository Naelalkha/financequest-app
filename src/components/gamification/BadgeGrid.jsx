...
const BadgeGrid = ({ badges = [], className = '' }) => {
  ...
  const sortedBadges = BADGE_DISPLAY_ORDER.map(badgeId => {
    ...
    return formatBadge(badgeId, currentLang, unlockedAt);
  }).filter(Boolean);
  ...
  Object.keys(BADGES).forEach(badgeId => {
    if (!BADGE_DISPLAY_ORDER.includes(badgeId)) {
      const unlockedAt = badgesSet.has(badgeId) 
        ? (badges.find(b => b === badgeId || (typeof b === 'object' && b.id === badgeId))?.unlockedAt || true)
        : null;
      sortedBadges.push(formatBadge(badgeId, currentLang, unlockedAt));
    }
  });
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {sortedBadges.map((badge, index) => (
          <motion.div
            ...
            className={`neon-card rounded-xl p-4 backdrop-blur-sm border-2 flex flex-col items-center gap-3 ${
              badge.unlocked
            ...
            {badge.unlocked ? (
              <div className="flex items-center gap-1 text-xs text-amber-400">
                ...
                {badge.unlockedAt && typeof badge.unlockedAt === 'string' ? (
                  <span>{t('gamification.badges.unlocked_on', { date: new Date(badge.unlockedAt).toLocaleDateString(currentLang === 'fr' ? 'fr-FR' : 'en-US') })}</span>
                ) : (
