      ...
      <div className="flex flex-wrap gap-3">
        {milestonesStatus.map((milestone, index) => (
          <motion.div
            ...
            className={`px-4 py-2 rounded-xl border-2 flex items-center gap-2 ${milestone.unlocked
          ...
          >
            ...
            <div className="text-center">
              <div className={`font-bold text-sm ${milestone.unlocked ? 'text-amber-300' : milestone.reached ? 'text-gray-300' : 'text-gray-500'
                }`}>
                {formatCurrency(milestone.amount)}
              </div>
              <div className="text-xs text-gray-500">
                {i18n.language === 'fr' ? '/an' : '/year'}
              </div>
            </div>
          </motion.div>
        ))}
