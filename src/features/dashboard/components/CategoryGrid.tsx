...
type CategoryId = 'PILOTAGE' | 'DEFENSE' | 'GROWTH' | 'STRATEGY';
...
const CategoryGrid: React.FC<CategoryGridProps> = ({ onSelectCategory }) => {
  ...
  const getQuestCount = (categoryId: CategoryId): number => {
    return (quests as Array<{ category?: string }>)?.filter(q => q.category === categoryId).length || 0;
  };
